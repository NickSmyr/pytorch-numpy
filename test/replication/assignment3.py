...
from framework.model import twolayer, ReLuDropoutBatchNormMLP, SkipMLP
from framework.non_pure import load_cifar, deterministic_seed
from framework.trainer import TriangularLRSchedule, Trainer
from framework.plotting import plot_loss_cost_accuracy_on_val_train
...
class Assignment2Replication(unittest.TestCase):
    ...
    def test_exercise2_2layer_should_get_over_52(self):
        self.exercise2_config([50,50], False)

    def test_exercise2_9layer_should_be_pretty_bad(self):
        self.exercise2_config([50, 30, 20, 20, 10, 10, 10, 10], False)

    def exercise2_config(self, hidden_dims, use_batchnorm):
        input_dim = 3072
        K = 10
        ...
        self.run_with_hyperparameters(K, None, input_dim, lamda,
                                      ...
                                      hidden_dims, use_batchnorm=False)
    ...
    def test_train_interface(self):
        ...
        self.run_with_hyperparameters(K, hidden_dim, input_dim,
                                      ...
                                      n_cycles,hidden_dims, use_batchnorm )
    ...
    def run_with_hyperparameters(self, K, hidden_dim, input_dim,
        ...
        if model == "twolayer":
            ...
            model = ReLuDropoutBatchNormMLP(input_dim, K, hidden_dims,
                               lamda=lamda,
                               pdrop=pdrop,
                               pflip=pflip,
                               ptrans=ptrans,
        ...
        res = trainer.train()
        print(res)
        plot_loss_cost_accuracy_on_val_train(res)
        model.set_test()
    ...
    def test_run_skip_mlp(self):
        trainX, trainY, valX, valY, testX, testY = load_cifar(
            "../../cifar-10-batches-py")
        ...
        batch_norm_test_scheme = "simple"  # "presicebn" "adaptivebn"
        ...
        config_str = f"{batch_norm_after_activation}"
        ...
        if model == "twolayer":
            ...
            model = SkipMLP(input_dim, K, hidden_dim, num_layers,
                            lamda, pdrop, pflip, ptrans, num_channels,
                            image_width, use_batchnorm, batch_norm_alpha,
                            image_height, initialization)
        # TODO move lr_scheudle in model
        if n_s is None:
            n_s = int(2 * trainX.shape[1] / batch_size)

        lr_schedule = TriangularLRSchedule(eta_min, eta_max, n_s)
        trainer = Trainer(model, trainX, trainY, valX,
                          valY, 0.001, None, 15, None,
                          batch_size, optimizer="sgd")
        trainer.attach_evaluators_on_epoch_end([
            ('accuracy', AccuracyEvaluator()),
            ('loss', twolayerLossEvaluator()),
            ('cost', twolayerCostEvaluator(use_batchnorm=use_batchnorm))
        ])
        res = trainer.train()
        plot_loss_cost_accuracy_on_val_train(res)
        model.set_test()

        Y_pred_train = model.forward(trainX)
        Y_pred_val = model.forward(valX)
        Y_pred_test = model.forward(testX)
        print("Final val accuracy ",
              evaluator.evaluate(model, valX, valY, Y_pred_val))
        print("Final train accuracy ",
              evaluator.evaluate(model, trainX, trainY, Y_pred_train)
              )
        print("Final test accuracy ",
              evaluator.evaluate(model, testX, testY, Y_pred_test))
